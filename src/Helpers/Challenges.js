...
export default class Challenges {
  ...
  static starter_challenges = [
    ...
    [ 'Drink ur Roll', 'Drink for %d seconds.' ],
    [ 'Guess a Song', '%p, %p, and %p hum songs. Guess each song correctly in 15 seconds or drink.' ],
    [ 'Nobody Laugh', 'Next person to laugh drinks.' ],
    [ 'Everyone Drink', 'Cheers!' ],
    ...
    [ 'Drink ur Roll Forever', 'Every time a %d gets rolled again, drink.' ],
    [ 'Everyone Drinks', 'Everybody drink!' ],
  ...
  static format(raw: string, state: syncState){
    // Replace all instances of the current player
    // Some weird logic here for error handling. TODO: Fix the shenanigans.
    raw = raw.replace(/%c/g, String(state.lastMove?.player ? state.players[state.lastMove.player].name : 'You'));
